...
static void MlmePollConfirm(MlmePollConfirmParams params) {
    if (params.m_status == LrWpanMlmePollConfirmStatus::MLMEPOLL_SUCCESS) {
        NS_LOG_UNCOND(Simulator::Now().GetSeconds() << " Polling SUCCESS");
    }
}
...
main(int argc, char* argv[])
{
    ...
    lrWpanHelper.EnablePcap(std::string("dsme-poll-dev0.pcap"), dev0, true, true);
    lrWpanHelper.EnablePcap(std::string("dsme-poll-dev1.pcap"), dev1, true, true);
    ...
    MlmePollConfirmCallback cb6;
    cb6 = MakeCallback(&MlmePollConfirm);
    dev1->GetMac()->SetMlmePollConfirmCallback(cb6);
    ...
    ///////////////////// Poll request //////////////////////    
    MlmePollRequestParams params3;
    params3.m_coorAddrMode = SHORT_ADDR;
    params3.m_coorPanId = 5;
    params3.m_coorShortAddr = Mac16Address("00:01");

    Simulator::ScheduleWithContext(1,
                                   Seconds(10),
                                   &LrWpanMac::MlmePollRequest,
                                   dev1->GetMac(),
                                   params3);
