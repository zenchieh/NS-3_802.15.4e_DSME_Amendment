...
int main(int argc, char** argv) {
    ...
    channelOffsets.push_back(6);

    uint16_t numOfChannelsSupported = 7;
    ...
    for (unsigned int i = 1; i < numOfCoord; ++i) {
        ...
        hoppingDescriptor.m_HoppingSequenceID = 0x00;
        hoppingDescriptor.m_hoppingSeqLen = 0;
        hoppingDescriptor.m_channelOfs = channelOffsets[i - 1];
        hoppingDescriptor.m_channelOfsBitmapLen = 16;
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(2)->GetObject<LrWpanNetDevice>(), true, 1, channelOffsets[1]
                                , superframeID, i);
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        // lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(3)->GetObject<LrWpanNetDevice>(), true, 1, superframeID, i);
        // lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>(), false, 1, superframeID, i);

        lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(3)->GetObject<LrWpanNetDevice>(), true, 1, channelOffsets[2]
                                , superframeID, i);
        lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>(), false, 1, channelOffsets[2]
                                , superframeID, i);

        lrWpanHelper.GenerateTraffic(lrwpanDevices.Get(childIdx), lrwpanDevices.Get(3)->GetAddress(), pktSize, 200, 100000.0, 125.8295);
    }
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        // Channel Offset setting
        lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>()->SetChannelOffset(channelOffsets[3]);

        // lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(4)->GetObject<LrWpanNetDevice>(), true, 1, superframeID, i);
        // lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>(), false, 1, superframeID, i);

        lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(4)->GetObject<LrWpanNetDevice>(), true, 1, channelOffsets[3]
                                , superframeID, i);
        lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>(), false, 1, channelOffsets[3]
                                , superframeID, i);

        lrWpanHelper.GenerateTraffic(lrwpanDevices.Get(childIdx), lrwpanDevices.Get(4)->GetAddress(), pktSize, 200, 100000.0, 125.8295);
    }
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        // Channel Offset setting
        lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>()->SetChannelOffset(channelOffsets[4]);

        // lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(5)->GetObject<LrWpanNetDevice>(), true, 1, superframeID, i);
        // lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>(), false, 1, superframeID, i);

        lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(5)->GetObject<LrWpanNetDevice>(), true, 1, channelOffsets[4]
                                , superframeID, i);
        lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>(), false, 1, channelOffsets[4]
                                , superframeID, i);

        lrWpanHelper.GenerateTraffic(lrwpanDevices.Get(childIdx), lrwpanDevices.Get(5)->GetAddress(), pktSize, 200, 100000.0, 125.8295);
    }
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        // Channel Offset setting
        lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>()->SetChannelOffset(channelOffsets[5]);

        // lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(6)->GetObject<LrWpanNetDevice>(), true, 1, superframeID, i);
        // lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>(), false, 1, superframeID, i);

        lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(6)->GetObject<LrWpanNetDevice>(), true, 1, channelOffsets[5]
                                , superframeID, i);
        lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>(), false, 1, channelOffsets[5]
                                , superframeID, i);

        lrWpanHelper.GenerateTraffic(lrwpanDevices.Get(childIdx), lrwpanDevices.Get(6)->GetAddress(), pktSize, 200, 100000.0, 125.8295);
    }
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        // Channel Offset setting
        lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>()->SetChannelOffset(channelOffsets[6]);

        // lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(7)->GetObject<LrWpanNetDevice>(), true, 1, superframeID, i);
        // lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>(), false, 1, superframeID, i);

        lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(7)->GetObject<LrWpanNetDevice>(), true, 1, channelOffsets[6]
                                , superframeID, i);
        lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>(), false, 1, channelOffsets[6]
                                , superframeID, i);

        lrWpanHelper.GenerateTraffic(lrwpanDevices.Get(childIdx), lrwpanDevices.Get(7)->GetAddress(), pktSize, 200, 100000.0, 125.8295);
    }
