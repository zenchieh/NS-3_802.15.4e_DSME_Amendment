...
BeaconIndication(MlmeBeaconNotifyIndicationParams params, Ptr<Packet> p)
{
    ...
    std::cout << " Mlme-BeaconNotification.indication. "
                          << "\n"
                          << " | Received BEACON packet of size | = " << p->GetSize()
                          << ", | Beacon Type | = " << std::hex << params.m_beaconType
                          << ", | Beacon Sequence Num | = " << std::hex << params.m_ebsn
                          << ", | Link Quality | = " << static_cast<int>(params.m_panDescriptor.m_linkQuality)
                          << ", | Timestamp | = " << params.m_panDescriptor.m_timeStamp
                          << params.m_panDescriptor.m_dsmeSuperframeSpec 
                          << params.m_panDescriptor.m_superframeSpec
                          << params.m_panDescriptor.m_bcnBitmap
                          << "\n";
}
...
main(int argc, char* argv[])
{
    ...
    // BeaconBitmap bcnBitmap;
    // bcnBitmap.SetSDIndex(0);
    // params.m_bcnBitmap = bcnBitmap;
    ...
    params2.m_txOptions |= TX_OPTION_DIRECT;
