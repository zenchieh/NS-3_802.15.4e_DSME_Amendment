...
int main(int argc, char** argv) {
    ...
    for (int j = 0; j < numOfChannelsSupported; ++j) {
        for (uint16_t superframeID = 0; superframeID <= 15; ++superframeID) {
            for (int i = 0; i < 7; ++i) {
    ...
    for (unsigned j = 0; j < numOfChannelsSupported; j++) {
        for (int i = 0; i < 7; ++i) {
            ...
            lrWpanHelper.GenerateTraffic(lrwpanDevices.Get(childIdx), lrwpanDevices.Get(j + 1)->GetAddress(), 60,  0, 100000.0, 7.86432);
        }
    }
