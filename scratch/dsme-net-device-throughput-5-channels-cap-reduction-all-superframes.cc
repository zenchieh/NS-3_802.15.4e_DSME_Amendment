...
int main(int argc, char** argv) {
    ...
    for (unsigned j = 0; j < numOfChannelsSupported; j++) {
        for (int i = 0; i < 7; ++i) {
            int childIdx = i + 6 + j * 15; 

            lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>()->SetChannelOffset(channelOffsets[j]);
    ...
    // superframe id = 1
    for (uint16_t superframeID = 1; superframeID <= 5; ++superframeID) {
        for (int i = 0; i < 15; ++i) {
            int childIdx = i + 6;
