...
int main(int argc, char** argv) {
    ...
    for (uint16_t superframeID = 0; superframeID <= 5; ++superframeID) {
        for (int i = 0; i < 7; ++i) {
            int childIdx = i + numOfCoord;
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        // lrWpanHelper.GenerateTraffic(lrwpanDevices.Get(childIdx), lrwpanDevices.Get(1)->GetAddress(), 60,  251.66022400, 100000.0, 7.86432);
    }


    for (uint16_t superframeID = 0; superframeID <= 5; ++superframeID) {
        for (int i = 0; i < 7; ++i) {
            int childIdx = i + numOfCoord + 7;
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        // lrWpanHelper.GenerateTraffic(lrwpanDevices.Get(childIdx), lrwpanDevices.Get(2)->GetAddress(), 60,  251.66022400, 100000.0, 7.86432);
    }

    for (uint16_t superframeID = 0; superframeID <= 5; ++superframeID) {
        for (int i = 0; i < 7; ++i) {
            int childIdx = i + numOfCoord + 14;

            // Channel Offset setting
            lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>()->SetChannelOffset(channelOffsets[3]);

            lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(3)->GetObject<LrWpanNetDevice>(), true, 1, channelOffsets[3]
                                    , superframeID, i);
    ...
    for (uint16_t superframeID = 0; superframeID <= 5; ++superframeID) {
        for (int i = 0; i < 7; ++i) {
            int childIdx = i + numOfCoord + 21;

            // Channel Offset setting
            lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>()->SetChannelOffset(channelOffsets[4]);

            lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(4)->GetObject<LrWpanNetDevice>(), true, 1, channelOffsets[4]
                                    , superframeID, i);
    ...
    for (uint16_t superframeID = 0; superframeID <= 5; ++superframeID) {
        for (int i = 0; i < 7; ++i) {
            int childIdx = i + numOfCoord + 28;

            // Channel Offset setting
            lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>()->SetChannelOffset(channelOffsets[5]);

            lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(5)->GetObject<LrWpanNetDevice>(), true, 1, channelOffsets[5]
                                    , superframeID, i);
    ...
    for (uint16_t superframeID = 0; superframeID <= 5; ++superframeID) {
        for (int i = 0; i < 7; ++i) {
            int childIdx = i + numOfCoord + 35;

            // Channel Offset setting
            lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>()->SetChannelOffset(channelOffsets[6]);

            lrWpanHelper.AddGtsInCfp(lrwpanDevices.Get(6)->GetObject<LrWpanNetDevice>(), true, 1, channelOffsets[6]
                                    , superframeID, i);
