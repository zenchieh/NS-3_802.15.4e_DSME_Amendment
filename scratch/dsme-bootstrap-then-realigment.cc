...
static void MlmeSyncLossIndication(Ptr<LrWpanNetDevice> device, MlmeSyncLossIndicationParams params) {
    if (params.m_lossReason == LrWpanSyncLossReason::MLMESYNCLOSS_SUPERFRAME_OVERLAP) {
        // TODO

    } else if (params.m_lossReason == LrWpanSyncLossReason::MLMESYNCLOSS_REALIGMENT) {
        std::cout << Simulator::Now().As(Time::S) << " Node " << device->GetNode()->GetId() << " ["
                  << device->GetMac()->GetShortAddress() << " | "
                  << device->GetMac()->GetExtendedAddress() << "]"
                  << " MLME-syncloss.indication: Received a coordinator realignment command.\n"
                  << " this device is no longer synchronized with the orginal coordinator\n"
                  << " this device should be realigned to new PanID: [" 
                  << params.m_panId
                  << "]"
                  << ", Channel Num: " 
                  << "["
                  << (uint32_t)params.m_logCh 
                  << "]"
                  << std::endl;

        // Re-SYNC
        MlmeSyncRequestParams syncParams;
        syncParams.m_logCh = params.m_logCh; 
        ...
        MlmeStartRequestParams reqParams;
        reqParams.m_panCoor = false;
        reqParams.m_PanId = params.m_panId;
        reqParams.m_bcnOrd = 6;
        reqParams.m_sfrmOrd = 3;
        reqParams.m_logCh = params.m_logCh;
        reqParams.m_coorRealgn = true;

        // Beacon Bitmap
        BeaconBitmap bitmap(0, 1 << (6 - 3));
        bitmap.SetSDIndex(8);                  // SD = 8 目前占用
        reqParams.m_bcnBitmap = bitmap;

        // Hopping Descriptor
        HoppingDescriptor hoppingDescriptor;
        hoppingDescriptor.m_HoppingSequenceID = 0x00;
        hoppingDescriptor.m_hoppingSeqLen = 0;
        hoppingDescriptor.m_channelOfs = 5;
        hoppingDescriptor.m_channelOfsBitmapLen = 16;
        hoppingDescriptor.m_channelOfsBitmap.resize(1, 34);    // offset = 1, 5 目前占用

        reqParams.m_hoppingDescriptor = hoppingDescriptor;

        // DSME
        reqParams.m_dsmeSuperframeSpec.SetMultiSuperframeOrder(6);
        reqParams.m_dsmeSuperframeSpec.SetChannelDiversityMode(1);
        reqParams.m_dsmeSuperframeSpec.SetCAPReductionFlag(false);
...
int main(int argc, char* argv[]) {
    ...
    endNode->AddDevice(secondCoordNetDevice);
    ...
    secondCoordNetDevice->GetMac()->SetMlmeSyncLossIndicationCallback(MakeBoundCallback(&MlmeSyncLossIndication
                                                                                    , secondCoordNetDevice));


    LrWpanHelper lrWpanHelper;
    
    lrWpanHelper.EnablePcap(std::string("dsme-bootstrap-then-realignment-coordNetDevice.pcap"), coordNetDevice, true, true);
    lrWpanHelper.EnablePcap(std::string("dsme-bootstrap-then-realignment-secondCoordNetDevice.pcap"), secondCoordNetDevice, true, true);
    ...
    params3.m_panCoor = true;
    params3.m_PanId = 77;
    params3.m_bcnOrd = 12;
    params3.m_sfrmOrd = 4;
    params3.m_logCh = 14;
    params3.m_coorRealgn = true;

    // Beacon Bitmap
    BeaconBitmap bitmap3(0, 1 << (12 - 4));
    bitmap3.SetSDBitmap(0);                  // SD = 0 目前占用
    params3.m_bcnBitmap = bitmap3;

    // Hopping Descriptor
    HoppingDescriptor hoppingDescriptor3;
    hoppingDescriptor3.m_HoppingSequenceID = 0x00;
    hoppingDescriptor3.m_hoppingSeqLen = 0;
    hoppingDescriptor3.m_channelOfs = 1;
    hoppingDescriptor3.m_channelOfsBitmapLen = 16;
    hoppingDescriptor3.m_channelOfsBitmap.resize(1, 2);    // offset = 1 目前占用

    params3.m_hoppingDescriptor = hoppingDescriptor3;

    // DSME
    params3.m_dsmeSuperframeSpec.SetMultiSuperframeOrder(10);
    params3.m_dsmeSuperframeSpec.SetChannelDiversityMode(1);
    params3.m_dsmeSuperframeSpec.SetCAPReductionFlag(true);
