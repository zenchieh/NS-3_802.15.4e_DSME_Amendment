...
int main(int argc, char** argv) {
    ...
    channelOffsets.push_back(7);
    channelOffsets.push_back(8);
    channelOffsets.push_back(9);
    channelOffsets.push_back(10);
    channelOffsets.push_back(11);
    channelOffsets.push_back(12);
    channelOffsets.push_back(13);
    channelOffsets.push_back(14);

    uint16_t numOfChannelsSupported = 15;
    ...
    for (unsigned j = 0; j < numOfChannelsSupported; j++) {
        for (int i = 0; i < 7; ++i) {
            int childIdx = i + numOfCoord + j * 15; 
    ...
    // superframe id = 1
    for (int j = 0; j < numOfChannelsSupported; ++j) {
        for (uint16_t superframeID = 1; superframeID <= 15; ++superframeID) {
            for (int i = 0; i < 15; ++i) {
                int childIdx = i + numOfCoord + j * 15;
                ...
                std::string addr = "00:0" + std::to_string(j + 2);
                lrwpanDevices.Get(childIdx)->GetObject<LrWpanNetDevice>()->GetMac()->SetAssociatedCoor(Mac16Address(const_cast<char*>(addr.c_str())));
            }
        }
    }
    ...
    for (unsigned j = 0; j < numOfChannelsSupported; j++) {
        for (int i = 0; i < 15; ++i) {
            ...
            lrWpanHelper.GenerateTraffic(lrwpanDevices.Get(childIdx), lrwpanDevices.Get(j + 1)->GetAddress(), 60, 251.65824, 100000.0, 7.86432);
        }
    }
